...
inline void first_syscall(CPUState *cpu, target_ulong pc, target_ulong callno) {
    osi_initialized = true; // Now callbacks will actually run
}
...
bool init_plugin(void *self) {
    ...
    osi_initialized = panda_parse_bool_opt(plugin_args, "load_now", "Immediately initialize OSI instead of waiting for first syscall");
    ...
    if (osi_initialized) {
      ...
      LOG_INFO(PLUGIN_NAME " deferring OSI initialization until first syscall.");
